   ...
   - _Theorem_: For any [n], [m],   n + m = m + n.
    ...
    - By induction on [m].
      Suppose [m = 0]. We must show
          S (q + 0) = 0 + (S q)
          S (0 + q) = 0 + (S q)
    - This follows from the definition of [+] and using 
      inductive hypothesis on [n]
      
    - Next, suppose [m = S w], where
         q + w = w + q -> S (q + w) = w + S q
    - We must show
         S (q + S w) = S w + S q
    - Applying inductive hypothesis of [m]:
         S (S w + q) = S (w + S q)

    - Using the proven theorem of [ S (n' + m') = n' + (S m') ] 
      on  n' = [S w] and m' = [q]:      
         S w + S q = S (w + S q)
      which follows immediatly from the definition of [+]. 
**)
